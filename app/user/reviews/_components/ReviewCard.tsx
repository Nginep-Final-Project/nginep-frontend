    ...
    <div className="bg-white shadow-md rounded-lg p-6">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex items-center">
          ...
          <span className="font-semibold">{review.averageRating.toFixed(1)}</span>
        </div>
      </div>
      <p className="text-gray-700 text-justify text-sm sm:text-base mb-4">{review.comment}</p>
      ...
      {review.reply && (
        <div className="mt-4 bg-gray-100 p-4 rounded-lg">
          <p className="font-semibold">Response from {review.reply.tenantName}:</p>
          <p className="text-gray-700 mt-2">{review.reply.reply}</p>
          <p className="text-gray-500 text-sm mt-2">
            Responded {formatDistanceToNow(new Date(review.reply.createdAt), { addSuffix: true })}
          </p>
        </div>
      )}
    </div>
